...
public class DB_Event {
	...
	public static Entity updateProperty(UpdateEventData data, Key eventKey, Entity event) {
		return Entity.newBuilder(eventKey).set(NAME, event.getString(NAME)).set(ID, event.getString(ID))
				...
				.set(CREATION_DATE, event.getString(CREATION_DATE)).set(CHAT, event.getList(CHAT))
	...
	public static Entity updateState(Key eventKey, Entity event, String state) {
		return Entity.newBuilder(eventKey).set(NAME, event.getString(NAME)).set(ID, event.getString(ID))
				...
				.set(CHAT, event.getList(CHAT)).set(PARTICIPANTS, event.getList(PARTICIPANTS))
				...
				.set(INSTAGRAM, event.getString(INSTAGRAM)).set(TWITTER, event.getString(TWITTER)).build();
	}
	...
	public static Entity updateProfile(Key eventKey, Entity event, String profile) {
		return Entity.newBuilder(eventKey).set(NAME, event.getString(NAME)).set(ID, event.getString(ID))
				...
				.set(STATE, event.getString(STATE)).set(PROFILE, profile)
				// .set(REGION, data.region)
				// .set(POSTAL_CODE, data.pc)
				.set(WEBSITE, event.getString(WEBSITE)).set(FACEBOOK, event.getString(FACEBOOK))
				.set(INSTAGRAM, event.getString(INSTAGRAM)).set(TWITTER, event.getString(TWITTER)).build();
	...
	public static Entity createNew(CreateEventData event_data, Key eventKey) {
		...
		return Entity.newBuilder(eventKey).set(NAME, data.name).set(ID, data.id).set(LOCATION, event_location)
				.set(START_DATE, data.start_date).set(END_DATE, data.end_date)
				.set(CREATION_DATE, Timestamp.now().toString()).set(CHAT, chat.build())
				...
				// .set(REGION, data.region)
				// .set(POSTAL_CODE, data.pc)
				.set(WEBSITE, data.website).set(FACEBOOK, data.facebook).set(INSTAGRAM, data.instagram)
	...
	public static Entity updateComment(Key eventKey, Entity event, String comment_id, String user_email,
			String comment_content) throws ImpossibleActionException, InexistentCommentIdException {
		List<Value<?>> chat = event.getList(DB_Event.CHAT);
	...
	public static Entity updateChat(Key eventKey, Entity event, ListValue newChat) {
		return Entity.newBuilder(eventKey).set(NAME, event.getString(NAME)).set(ID, event.getString(ID))
				.set(LOCATION, event.getLatLng(LOCATION)).set(START_DATE, event.getString(START_DATE))
				.set(END_DATE, event.getString(END_DATE)).set(CREATION_DATE, event.getString(CREATION_DATE))
				.set(CHAT, newChat).set(PARTICIPANTS, event.getList(PARTICIPANTS))
				.set(N_PARTICIPANTS, event.getLong(N_PARTICIPANTS)).set(OWNER_EMAIL, event.getString(OWNER_EMAIL))
				.set(CONTACT, event.getString(CONTACT)).set(DESCRIPTION, event.getString(DESCRIPTION))
				.set(CATEGORY, event.getString(CATEGORY)).set(CAPACITY, event.getLong(CAPACITY))
	...
	public static Entity updateParticipants(Key eventKey, Entity event, ListValue newParticipants, boolean add) {
		...
		return builder.set(NAME, event.getString(NAME)).set(ID, event.getString(ID))
				.set(LOCATION, event.getLatLng(LOCATION)).set(START_DATE, event.getString(START_DATE))
				.set(END_DATE, event.getString(END_DATE)).set(CREATION_DATE, event.getString(CREATION_DATE))
				.set(CHAT, event.getList(CHAT)).set(PARTICIPANTS, newParticipants)
