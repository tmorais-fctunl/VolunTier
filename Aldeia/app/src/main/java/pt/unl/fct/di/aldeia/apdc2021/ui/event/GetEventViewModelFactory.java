...
import pt.unl.fct.di.aldeia.apdc2021.data.LeaveEventDataSource;
...
public class GetEventViewModelFactory implements ViewModelProvider.Factory {
    ...
    public <T extends ViewModel> T create(@NonNull Class<T> modelClass) {
        if (modelClass.isAssignableFrom(GetEventViewModel.class)) {
            return (T) new GetEventViewModel(GetEventRepository.getInstance(new GetEventDataSource(), new UpdateEventDataSource(), new RemoveEventDataSource(),new ParticipateEventDataSource(), new LeaveEventDataSource()), executor);
        } else {
